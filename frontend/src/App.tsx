...
import { CheckDependencies, DownloadDependencies } from "../wailsjs/go/main/App"
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        setCheckingDeps(false)
        setDepError('Failed to check dependencies. Check your connection.')
      }
    }

    const handleProgress = (data: DepProgress) => {
      setDepProgress((prev) => ({
        ...prev,
    ...
    EventsOn('dependency-progress', handleProgress)
    run()
      ...
      <div className="h-screen flex flex-col items-center justify-center bg-surface text-gray-200 px-6">
        ...
        {installingDeps && (
          <p className="text-xs text-gray-500 mt-4 font-mono">
            {depProgress['yt-dlp'] === 100 && depProgress['ffmpeg'] === 100
              ? 'Finalizing...'
              : 'Downloading...'}
          </p>
        )}
        ...
        {depError && (
          <div className="mt-4 text-center">
            ...
            <button onClick={handleInstallDeps} className="btn-primary text-xs px-4 py-1.5">
              Retry
            </button>
          </div>
        )}
